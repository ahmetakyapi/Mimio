...
export function WeeklyProgressReport({ client, sessions, goals, therapistName, clinicName, allNotes }: WeeklyProgressReportProps) {
  ...
  function handlePrint() {
    ...
    const trendColors: Record<string, string> = { up: "#12b886", down: "#d63d63", stable: "#f59e0b", new: "#2b62f5" };
    ...
    const goalRows = [...report.activeGoals, ...report.completedGoals].map(g => {
      ...
      return `<tr><td>${g.title}</td><td><div style="display:flex;align-items:center;gap:8px"><div style="flex:1;height:8px;background:#eaf2ff;border-radius:4px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${isComplete ? "#12b886" : "#2b62f5"};border-radius:4px"></div></div><span style="font-weight:700;color:${isComplete ? "#12b886" : "#2b62f5"}">${pct}%</span></div></td><td>${g.deadline ?? "—"}</td></tr>`;
    }).join("");
    ...
    const html = `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><title>Haftalık İlerleme — ${client.displayName}</title><style>
      body{font-family:'Segoe UI',Arial,sans-serif;color:#3d5670;margin:0;padding:28px;font-size:13px;line-height:1.5}
      h1{font-size:22px;margin:0 0 4px;color:#3d5670}
      h2{font-size:13px;font-weight:800;margin:24px 0 8px;color:#2b62f5;text-transform:uppercase;letter-spacing:.08em;border-bottom:2px solid #dbe6fb;padding-bottom:4px}
      .header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #2b62f5;padding-bottom:16px;margin-bottom:24px}
      .badge{display:inline-block;background:#dbe6fb;color:#2b62f5;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;margin-right:4px}
      table{width:100%;border-collapse:collapse;margin-bottom:12px}td,th{padding:8px 12px;border:1px solid #cddcf0;text-align:left;font-size:12px}th{background:#eef3fa;font-weight:700;color:#1b4bc4}
      .stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}
      .stat{background:#eef3fa;border:1px solid #dbe6fb;border-radius:12px;padding:14px;text-align:center}
      .stat-val{font-size:28px;font-weight:900;color:#2b62f5}.stat-lbl{font-size:10px;color:#7b91ab;font-weight:600;text-transform:uppercase;letter-spacing:.05em}
      .trend-up{color:#12b886}.trend-down{color:#d63d63}.trend-stable{color:#f59e0b}
      .note-list{padding-left:18px;font-size:12px;color:#6b83a0}
      .footer{margin-top:32px;padding-top:12px;border-top:2px solid #cddcf0;font-size:10px;color:#7b91ab;text-align:center}
      @media print{body{padding:0}button{display:none}}
    </style></head><body>
      <div class="header">
        <div>
          <h1>Haftalık İlerleme Raporu</h1>
          <p style="color:#2b62f5;font-weight:700;font-size:14px;margin:4px 0">${client.displayName}</p>
        ...
        <div style="text-align:right;font-size:11px;color:#94a8bf">
          <strong style="font-size:13px;color:#3d5670">${therapistName}</strong>
  ...
  const trendColor = report.scoreDelta > 5 ? "#12b886" : report.scoreDelta < -5 ? "#d63d63" : "#f59e0b";
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "rgba(43, 98, 245,0.12)" }}>
              <Calendar size={15} style={{ color: "#4d7dff" }} />
            </div>
            <div>
          ...
          <button type="button" onClick={handlePrint}
            ...
            style={{ background: "rgba(43, 98, 245,0.1)", color: "#4d7dff", border: "1px solid rgba(43, 98, 245,0.2)" }}>
          ...
          {[
            ...
            <div key={label} className="rounded-xl p-2.5 text-center" style={{ background: "var(--color-surface-elevated)" }}>
              ...
              {typeof prev === "number" && prev > 0 && typeof value === "number" && (
                <span className="text-[8px] block font-bold" style={{ color: value > prev ? "#12b886" : value < prev ? "#d63d63" : "#f59e0b" }}>
        ...
        {report.gamePerformance.length > 0 && (
          <div className="space-y-2 mb-4">
            ...
            {report.gamePerformance.map(g => {
              ...
              const tColor = g.trend === "up" ? "#12b886" : g.trend === "down" ? "#d63d63" : "#f59e0b";
              return (
                <div key={g.key} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-4 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
                    <div className="h-full rounded-full flex items-center justify-end pr-1.5 bar-animated" style={{ "--bar-width": `${Math.max(pct, 8)}%`, background: "linear-gradient(90deg, rgba(43, 98, 245,0.5), #2b62f5)" } as React.CSSProperties}>
                ...
                return (
                  <div key={g.id} className="flex items-center gap-2">
                    ...
                    <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                      <div className="h-full rounded-full" style={{ width: `${pct}%`, background: "#2b62f5" }} />
                    </div>
                    <span className="text-[10px] font-bold tabular-nums" style={{ color: "#2b62f5" }}>%{pct}</span>
                  </div>
                );
              })}
