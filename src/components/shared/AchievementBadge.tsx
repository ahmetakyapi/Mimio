...
export const ACHIEVEMENTS: AchievementDef[] = [
  // Session milestones
  { id: "first-session", title: "İlk Adım", description: "İlk seans tamamlandı", icon: Star, color: "#3f7d4f", gradient: "linear-gradient(135deg, #3f7d4f, #33663f)", category: "session", check: (s) => s.totalSessions >= 1 },
  { id: "ten-sessions", title: "Düzenli Terapist", description: "10 seans tamamlandı", icon: Award, color: "#1d5a8c", gradient: "linear-gradient(135deg, #1d5a8c, #17456e)", category: "session", check: (s) => s.totalSessions >= 10 },
  { id: "fifty-sessions", title: "Deneyimli Uzman", description: "50 seans tamamlandı", icon: Trophy, color: "#b8763a", gradient: "linear-gradient(135deg, #b8763a, #8f5626)", category: "session", check: (s) => s.totalSessions >= 50 },
  { id: "hundred-sessions", title: "Yüzüncü Seans", description: "100 seans başarıyla tamamlandı!", icon: Trophy, color: "#b8503f", gradient: "linear-gradient(135deg, #b8503f, #8a2c21)", category: "milestone", check: (s) => s.totalSessions >= 100 },

  // Score achievements
  { id: "high-scorer", title: "Yüksek Performans", description: "Tek seansta 200+ puan", icon: Zap, color: "#b8763a", gradient: "linear-gradient(135deg, #b8763a, #8f5626)", category: "score", check: (s) => s.bestScore >= 200 },
  { id: "perfect-game", title: "Kusursuz Oyun", description: "Bir oyunda tüm turlar doğru", icon: Sparkles, color: "#2a72ac", gradient: "linear-gradient(135deg, #2a72ac, #17456e)", category: "score", check: (s) => s.perfectGames >= 1 },

  // Streak achievements
  { id: "three-day-streak", title: "Üç Gün Serisi", description: "3 gün üst üste seans yapıldı", icon: Flame, color: "#a8392c", gradient: "linear-gradient(135deg, #a8392c, #8a2c21)", category: "streak", check: (s) => s.sessionStreak >= 3 },
  { id: "week-streak", title: "Haftalık Seri", description: "7 gün üst üste seans yapıldı", icon: Flame, color: "#b8763a", gradient: "linear-gradient(135deg, #b8763a, #8f5626)", category: "streak", check: (s) => s.sessionStreak >= 7 },

  // Diversity achievements
  { id: "explorer", title: "Kaşif", description: "Tüm oyun türlerini denedi", icon: Target, color: "#5b7183", gradient: "linear-gradient(135deg, #5b7183, #465a6b)", category: "milestone", check: (s) => s.uniqueGamesPlayed >= 7 },

  // Therapist achievements
  { id: "caring-therapist", title: "İlgili Terapist", description: "10+ seans notu yazıldı", icon: Heart, color: "#b8503f", gradient: "linear-gradient(135deg, #b8503f, #8a2c21)", category: "special", check: (s) => s.notesWritten >= 10 },
  { id: "goal-setter", title: "Hedef Odaklı", description: "5 hedef tamamlandı", icon: TrendingUp, color: "#3f7d4f", gradient: "linear-gradient(135deg, #3f7d4f, #33663f)", category: "special", check: (s) => s.goalsCompleted >= 5 },
  { id: "weekly-warrior", title: "Haftalık Savaşçı", description: "Bir haftada 10+ seans", icon: Clock, color: "#2a72ac", gradient: "linear-gradient(135deg, #2a72ac, #17456e)", category: "milestone", check: (s) => s.thisWeekSessions >= 10 },
];
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border p-4" style={{ background: "var(--color-surface-strong)", borderColor: "var(--color-line)" }}>
        ...
        <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${progressPct}%`, background: "linear-gradient(90deg, #1d5a8c, #2a72ac, #b8503f)" }} />
        </div>
