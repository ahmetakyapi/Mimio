...
export function SessionSetSummary({ sessionSet, onClose, onNewSet }: SessionSetSummaryProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(8,13,12,0.92)", backdropFilter: "blur(8px)" }}>
      <div className="relative w-full max-w-md mx-4 rounded-3xl overflow-hidden" style={{ background: "linear-gradient(160deg, #0d1424 0%, #08111f 100%)", border: "1px solid rgba(15, 110, 99,0.25)", boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}>
        <div className="h-1 w-full" style={{ background: "linear-gradient(90deg, #0f6e63, #35b0a0, #c4614a)" }} />

        <div className="p-6 space-y-5">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0" style={{ background: "linear-gradient(135deg, rgba(15, 110, 99,0.2), rgba(53, 176, 160,0.1))", border: "1px solid rgba(15, 110, 99,0.3)" }}>
              🏁
            </div>
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#35b0a0] m-0">{sessionSet.presetLabel} · Tamamlandı</p>
          ...
          <div className="rounded-2xl p-4 text-center" style={{ background: "rgba(15, 110, 99,0.08)", border: "1px solid rgba(15, 110, 99,0.2)" }}>
            <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold m-0 mb-1">Toplam Skor</p>
            <span className="font-extrabold tabular-nums" style={{ fontSize: "3.5rem", lineHeight: 1, background: "linear-gradient(135deg, #35b0a0, #35b0a0)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              {total}
            </span>
          ...
          <div className="space-y-2">
            {sessionSet.entries.map((entry, i) => {
              const accent = (["#7fb2cc", "#8fbdd6", "#7fb2cc", "#8fbdd6", "#9ac7dc", "#a9d1e2", "#35b0a0"] as const)[i % 7];
              return (
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onNewSet}
              ...
              style={{ background: "linear-gradient(135deg, #0f6e63, #35b0a0)", boxShadow: "0 4px 20px rgba(15, 110, 99,0.35)" }}>
              ↩ Yeni Set
            </button>
