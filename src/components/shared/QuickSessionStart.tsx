...
export function QuickSessionStart({ clients, activeClientId, recentSessions, onSelectClient, onStartGame, onStartSessionSet }: QuickSessionStartProps) {
  ...
  const quickActions = [
    lastGame && { label: `Devam: ${GAME_LABELS[lastGame]}`, icon: RotateCcw, gameKey: lastGame, color: "#1d5a8c", description: "Son oynanan oyun" },
    leastPlayed && { label: `Önerilen: ${GAME_LABELS[leastPlayed]}`, icon: Star, gameKey: leastPlayed, color: "#b8763a", description: "Az çalışılan alan" },
    { label: "Hızlı Set", icon: Zap, preset: "quick-mix", color: "#3f7d4f", description: "3 oyunluk karma set" },
  ...
  return (
    <div className="rounded-2xl sm:rounded-3xl border overflow-hidden" style={{
      background: "linear-gradient(135deg, rgba(29, 90, 140,0.06), rgba(42, 114, 172,0.04))",
      borderColor: "rgba(29, 90, 140,0.15)",
    }}>
      <div className="p-4 sm:p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #1d5a8c, #2a72ac)", boxShadow: "0 4px 12px rgba(29, 90, 140,0.3)" }}>
        ...
        {activeClient && (
          <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl" style={{ background: "var(--color-surface-strong)", border: "1px solid var(--color-line)" }}>
            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-extrabold text-white shrink-0"
              style={{ background: "linear-gradient(135deg, #2a72ac, #1d5a8c)" }}>
